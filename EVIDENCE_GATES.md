...
| Independent verification | Partial: energy gate cross-checked | `energy_independent_check.py` independently reproduces all three committed load-factor rows within 1e-9; other gates remain internally tested only | Results outside the energy gate may still contain implementation or transcription errors | Add independent calculations for thermal, speed-tradeoff, and airport summaries |
