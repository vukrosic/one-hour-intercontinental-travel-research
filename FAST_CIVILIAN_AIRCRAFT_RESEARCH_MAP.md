...
- **Current subsonic references:** `CURRENT_CIVILIAN_SPEED_CLOSE_READ.md` separates a Mach 0.85 airline-service baseline from three certified in-service business-aviation references at Mach 0.925, Mach 0.935, and Mach 0.95 (Global 7500, G700, and Global 8000).
...
Speed alone is not the research conclusion. `CURRENT_SPEED_TIME_CONTEXT.md` shows that the certified Global 7500, G700, and Global 8000 references save about 8.1%, 9.1%, and 10.5% of speed-only airborne time versus the Mach 0.85 airline baseline, while the historical Mach 2 class saves 57.5% in the same arithmetic screen. A Mach 2-class cruise has the only direct historical supersonic passenger-service precedent in this ledger, while current high-speed programmes remain research, experimental-test, company-reported, or proposed-regulatory evidence. The current subsonic frontier also splits by service class: airline operations cluster near Mach 0.85, while certified business aviation reaches higher subsonic values. The fastest practical civilian aircraft is bounded by noise acceptance, energy per passenger, passenger comfort, airport compatibility, and certification evidence. Faster abstract speed bands are hypotheses to test, not recommendations. The next useful result is therefore a gate-by-gate evidence matrix, not a faster notional aircraft.
